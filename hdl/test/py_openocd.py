...
def run(args):
    ...
    try:
        ...
        data, response = bus_read32(jtag, args.addr)
        ...
        if args.expect_data is not None and data != args.expect_data:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Tiny OpenOCD-like remote-bitbang client for BlueJ tests")
    parser.add_argument("--socket", default="/tmp/jtag.sock")
