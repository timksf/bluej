...
typedef enum { OKAY, ERROR, INVALID } BusResponseCode_t deriving(FShow, Eq, Bits);
...
module mkJTAG_BusAdapterCore#(Clock bus_clk, Reset bus_rst)(JTAG_BusAdapterCore_ifc#(aw, dw));
    ...
    rule r_queue_req if(jrg_bus_ctrl.wr_o() && !jrg_bus_ctrl.reg_o().ignore && !jrg_ctrl_i.busy);
