...
typedef struct {
    Bit#(n) blink_en;
    Bit#(w) blink_div;
} JTAG_LED_Ctrl#(numeric type n, numeric type w) deriving (Eq, Bits);
...
module mkFPGATestSimpleTop(FPGATest_ifc);
    ...
    */
    ...
    Reg#(JTAG_LED_Ctrl#(`N_LEDS, 16)) rg_led_ctrl <- mkRegU;
    JTAG_Reg_ifc#(JTAG_LED_Ctrl#(`N_LEDS, 16)) jtag_led_ctrl <- mkJTAG_Reg_Sync(rg_led_ctrl, sys_clk, sys_rst, clocked_by tck, reset_by trst);
    ...
    rule rtoggle if(rg_div == zeroExtend(rg_led_ctrl.blink_div));
        ...
        rg_leds <= (rg_leds & rg_led_ctrl.blink_en) ^ rg_leds;
    endrule
